...
3. Run `arena` with at least two read-only architecture candidates selected for the decision. Use `council-sol` for fresh Sol judgment, `reviewer` for a fresh Grok counterpoint, and `oracle` only when inherited context is evidence. Give every candidate the same brief and `references/runner-prompt.md`. Require the package in `references/rationale-template.md` and screen against `references/design-red-flags.md`.
