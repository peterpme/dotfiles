...
3. Launch at least two candidates through one async `workflowScript`. For read-only design or prose artifacts, choose dynamically among fresh `council-sol`, fresh `reviewer`, and forked `oracle`; include `oracle` only when inherited context is evidence. For writable code artifacts, use fresh `worker` or `petey-agent` roles and give every writer a separate worktree. Do not select models per run.
