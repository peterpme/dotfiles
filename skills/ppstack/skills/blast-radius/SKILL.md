...
6. For a big or wide change, run it as an `arena`. Use **spawn-subagent** under [the delegation contract](../../docs/subagents.md) to ask fresh helpers the same scoped question. Give them the evidence and task instructions, with no edits or further delegation. Merge their findings, and use **peer-review** when actual different families are needed. Independent passes catch different real bugs.
