...
- **CLI shape:** one executable with subcommands (`doctor`, `snapshot`, `screenshot <path>`, `press`, `click`, `wait-settle`, `console`, `reset-db`, `cleanup`), so an agent runs a command instead of writing a throwaway script (the **build-the-lever** principle skill). Output machine-readable JSON by default. Give every destructive command (`reset-db`, `cleanup`, anything that skips or seeds state) a `--dry-run`. Error messages say what to run instead. Rich `--help` on every subcommand. Hide the harness behind few deep commands rather than many shallow ones.
