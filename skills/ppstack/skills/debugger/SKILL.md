...
description: >-
  ...
  bash and read calls on its own; the model calls the log_problem
...
- **The extension** (`pi/extensions/debugger`) appends a row for every failed `bash` or `read` call, skipping lookups that fail by design (`rg`, `grep`, `fd`, `find`, `ls`, `git diff`) and repeating an identical failure once per session. No model tokens.
