...
2. **Source wave.** Load **spawn-subagent** and follow [the delegation contract](../../docs/subagents.md). Launch one child per feature file with no-edits and no-further-delegation instructions, source paths, and a standalone brief. The parent runs this skill and sends each helper only its feature file and source-reading task. Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe. Children never drive the app and never edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe.
