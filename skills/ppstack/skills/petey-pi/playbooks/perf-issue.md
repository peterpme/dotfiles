...
1. Capture a baseline trace via the best installed product-surface verifier.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation through **spawn-subagent** under [the delegation contract](../../../docs/subagents.md). Supply a neutral brief with the trace, scope, measurement command, and portable skill paths. The parent reads while the child writes, then reviews its diff. Capture a post-fix trace.
