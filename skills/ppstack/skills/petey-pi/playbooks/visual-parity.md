...
3. Migrate one component at a time. Use one writer per checkout. Parallel component implementations require worktrees explicitly requested or approved by the user, under [the delegation contract](../../../docs/subagents.md). Shared primitives migrate first as a blocking phase.
4. Verify each component against its baseline via image diff on the matching surface through the best installed product-surface verifier. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Iterate per component until the diff is zero.
