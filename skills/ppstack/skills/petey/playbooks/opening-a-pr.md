...
**Worktree.** Work from a git worktree off main; subagents inherit it. Multiple Pi writers each get their own managed worktree. Never let concurrent writers share a checkout. Dirty branch with unrelated work: patch out, fresh worktree, apply. Snarled worktree: reset from main, redo minimally.
...
**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, or `perf` as the type. Use the changed area, such as `ppstack` or `petey`, as the scope. Keep the subject short and imperative. Apply the same **technical-writing** and **unslop** pass as the body. Name a real symbol when one carries the change. For example, `fix(petey): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
A subagent that opens a PR runs `interrogate`, a focused cleanup pass, **no-comments**, and **no-stupid-tests** when the diff touches tests. It returns the URL and does not babysit. Return to the parent.
