...
Pi sessions live under the active workspace's directory in `~/.pi/agent/sessions/`; `/session` and `PI_SESSION_FILE` expose the current file. Every line is one JSONL session record. Stay inside the active workspace's session directory and never scan other projects without being asked.

1. Classify, then route. One specific prior chat to resume is the `session-pickup` playbook, not this. Turning habits into a durable skill is the **reflect** skill. A human-readable summary of your work is a different task. Recall loads working context across recent chats before you act. If the user already gave you a full state capsule (paths, branch, the change), use it and skip the mining.
...
3. Fan out across your chat history. Load **spawn-subagent** and follow [the delegation contract](../../docs/subagents.md). Launch fresh children with no-edits instructions, each taking a named slice of the corpus. The parent runs this skill. Give helpers the transcript paths and the concrete search and report instructions below, with no further delegation. Tell every subagent to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
