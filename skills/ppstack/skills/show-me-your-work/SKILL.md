...
At the end of the run, before handing back, check the log told the truth. Read this run's Pi session from `PI_SESSION_FILE` or the path reported by `/session`. Stay inside the active workspace's session directory and never scan unrelated projects. Walk the log against what actually happened:
